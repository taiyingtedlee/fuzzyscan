...
//??? exec_action() ???//
							if((e = exec_action(op2_minor, &buf[4], cmd[4])) <0 ){ // ?! LOOKUP func e=exec_action <0 ; false 
								err = 16;
								goto err_nak;
							}
							if(e==0) {// LOOKUP func e=exec_action = 0 ; true
								goto no_ack;
							}
							goto do_finish;
						}
							...
							if(cmd[1]& 0x40){ // 0x4,5,6,7,C,D,E,F ; if cmd[1] is GET cmd
								int rn;
//??? send_bytes() ???//
								rn = send_bytes(op1_major,op2_minor, result_buf1, send_len); // ?! LOOKUP func send_bytes() 
...
//??? send_len >> 0 & 0xff ???// 		
		...
		if ((cmd[4] & SERCMD_NO_ACKNAK) == 0){ // ?! SERCMD_NO_ACKNAK 0x02 ; Bit 2 ACK/NAK 0 = Request ; 1 = Do NOT Request
			printf("send_ack_packet : 7e 0f 00 00 00 00 00 0f 7e\n");
...
//	nak_reason = err; //  LOOKUP nak_reason :  static unsigned char nak_reason; is used in static int get_nak_status (){};
	...
	if ((cmd[4] & SERCMD_NO_ACKNAK) == 0)
		printf("send_nak_packet : 7e 0e 00 xx 00 00 00 0e 7e");
